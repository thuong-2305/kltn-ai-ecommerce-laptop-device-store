...
def _serialize_product(request, product, sale=None):
    ...
    thumbnails = [
        url
        for url in (
            _build_media_url(request, thumbnail.image)
    ...
    try:
        ...
        qs = (
            ...
            .annotate(
                average_rating=Avg("review__rating", filter=Q(review__is_spam=False)),
                review_count=Count(
                    "review", filter=Q(review__is_spam=False), distinct=True
                ),
                total_sold=Sum("orderitem__quantity"),
            )
        )
        ...
        if cpu:
            qs = qs.filter(
                Q(config__icontains=cpu)
                | Q(
                    specifications__key__name__iexact="CPU",
                    specifications__value__icontains=cpu,
                )
            ).distinct()
        ...
        if ram:
            qs = qs.filter(
                Q(config__icontains=ram)
                | Q(
                    specifications__key__name__iexact="RAM",
                    specifications__value__icontains=ram,
                )
            ).distinct()
        ...
        if storage:
            qs = qs.filter(
                Q(config__icontains=storage)
                | Q(
                    specifications__key__name__iexact="Ổ cứng",
                    specifications__value__icontains=storage,
                )
                | Q(
                    specifications__key__name__iexact="Storage",
                    specifications__value__icontains=storage,
                )
            ).distinct()
        ...
        if screen:
            qs = qs.filter(
                Q(config__icontains=screen)
                | Q(
                    specifications__key__name__iexact="Màn hình",
                    specifications__value__icontains=screen,
                )
                | Q(
                    specifications__key__name__iexact="Screen",
                    specifications__value__icontains=screen,
                )
            ).distinct()
        ...
        if os_param:
            qs = qs.filter(
                Q(config__icontains=os_param)
                | Q(
                    specifications__key__name__iexact="Hệ điều hành",
                    specifications__value__icontains=os_param,
                )
                | Q(
                    specifications__key__name__iexact="OS",
                    specifications__value__icontains=os_param,
                )
            ).distinct()
        ...
        try:
            ...
            qs = qs.filter(
                effective_price__gte=min_price, effective_price__lte=max_price
            )
        except (ValueError, TypeError):
            pass
        ...
        results = [
            _serialize_product(
                request, product, active_sales_by_category.get(product.category_id)
            )
        ...
        return JsonResponse(
            {"error": "Lỗi hệ thống. Vui lòng thử lại sau.", "results": []}, status=500
        )
...
def categories_api(request):
    ...
    try:
        ...
        return JsonResponse(
            {"results": data}, json_dumps_params={"ensure_ascii": False}
        )
    except Exception as exc:
...
def brands_api(request):
    ...
    try:
        ...
        data = [_serialize_brand(request, b) for b in brands]
        return JsonResponse(
            {"results": data}, json_dumps_params={"ensure_ascii": False}
        )
    except Exception as exc:
...
def product_detail_api(request, pk):
    ...
    specifications = [
        {"id": spec.id, "key": spec.key.name, "value": spec.value}
    ...
    review_qs = (
        Review.objects.filter(product=product, is_spam=False)
        .select_related("user")
...
def home_api(request):
    ...
    payload = {
        "categories": [
            _serialize_category(
                request, category, active_sales_by_category.get(category.id)
            )
            for category in categories
        ],
        "discounted_category_ids": list(active_sales_by_category.keys()),
        "active_sales": [_serialize_sale(request, sale) for sale in active_sales],
        "featured_products": [
            _serialize_product(
                request, product, active_sales_by_category.get(product.category_id)
            )
...
def search_by_image_api(request):
    ...
    if not image_file:
        return Response(
            {"error": "Vui lòng cung cấp hình ảnh để tìm kiếm"},
    ...
    try:
        ...
        return Response(
            {"error": "Lỗi hệ thống khi tìm kiếm. Vui lòng thử lại sau."},
