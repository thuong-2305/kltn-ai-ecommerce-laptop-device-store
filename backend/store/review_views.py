...
def submit_review_api(request):
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Dữ liệu product_id hoặc rating không hợp lệ"}, status=400
        )
    ...
    if Review.objects.filter(user=user, product=product).exists():
        return JsonResponse(
            {"error": "Bạn đã đánh giá sản phẩm này rồi và không thể chỉnh sửa."},
            status=400,
        )
    ...
    from store.sentiment import sentiment_analyzer

    sentiment_label, confidence = sentiment_analyzer.analyze(
        full_comment, rating=rating
    )
    ...
    if uploaded_files:
        ...
        for file in uploaded_files[:5]:
            if file.size > MAX_REVIEW_IMAGE_SIZE:
                return JsonResponse(
                    {"error": f'Kích thước ảnh "{file.name}" vượt quá 5MB'}, status=400
                )
        if not created:
    ...
    return JsonResponse(
        {
            "message": (
                ...
                if created
                else "Đánh giá đã được cập nhật!"
            ),
...
def admin_reviews_api(request):
    ...
    if not (user.is_staff or user.is_superuser):
        return JsonResponse(
            {"error": "Bạn không có quyền truy cập thông tin này"}, status=403
        )
        ...
        serialized.append(
            {
                ...
                "time": (
                    r.review_date.strftime("%d/%m/%Y %H:%M") if r.review_date else ""
                ),
...
def product_sentiment_stats_api(request, pk):
    ...
    reviews = Review.objects.filter(product=product, is_spam=False).order_by(
        "-review_date"
    )
    ...
    for i in range(6, -1, -1):
        ...
        if day_total > 0:
            pos_pct = round(
                (day_reviews.filter(sentiment="positive").count() / day_total) * 100
            )
            neu_pct = round(
                (day_reviews.filter(sentiment="neutral").count() / day_total) * 100
            )
            neg_pct = round(
                (day_reviews.filter(sentiment="negative").count() / day_total) * 100
            )
        else:
    ...
    aspect_list = []
    product_avg = (
        round(float(reviews.aggregate(avg=Avg("rating"))["avg"] or 0.0), 1)
        if total_reviews > 0
        else 4.0
    )
    ...
    for name, keywords in aspect_keywords.items():
        ...
        score = (
            round(aspect_total_rating / aspect_mention_count, 1)
            if aspect_mention_count > 0
            else product_avg
        )
        aspect_list.append(
            {
...
def global_sentiment_stats_api(request):
    ...
    products = (
        Product.objects.annotate(
            ...
            pos_count=Count(
                "review", filter=Q(review__sentiment="positive", review__is_spam=False)
            ),
            neu_count=Count(
                "review", filter=Q(review__sentiment="neutral", review__is_spam=False)
            ),
            neg_count=Count(
                "review", filter=Q(review__sentiment="negative", review__is_spam=False)
            ),
        )
        ...
        serialized_products.append(
            {
                ...
                "positive_percent": (
                    round((p.pos_count / total) * 100) if total > 0 else 0
                ),
                "neutral_percent": (
                    round((p.neu_count / total) * 100) if total > 0 else 0
                ),
                "negative_percent": (
                    round((p.neg_count / total) * 100) if total > 0 else 0
                ),
            }
        )
    ...
    if request.method == "PUT":
        try:
            ...
            if is_spam is not None:
                ...
                review.save()
            return JsonResponse(
                {"success": True, "message": "Cập nhật bình luận thành công"}
            )
