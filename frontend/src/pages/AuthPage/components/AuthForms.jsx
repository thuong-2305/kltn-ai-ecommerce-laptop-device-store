...
function Field({ label, icon: Icon, error, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className={`relative flex items-center rounded-xl border bg-white transition-all ${error ? 'border-red-400 shadow-[0_0_0_3px_rgba(239,68,68,0.12)]' : 'border-slate-200 focus-within:border-blue-500 focus-within:shadow-[0_0_0_3px_rgba(37,99,235,0.1)]'
        }`}>
        {Icon && (
  ...
  const handleGoogle = () => {
    ...
    if (window.google?.accounts?.oauth2) {
      const tokenClient = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (response) => {
          if (response.error) {
            setGlobalError('Đăng nhập Google thất bại hoặc bị hủy.')
            return
          }
          if (response.access_token) {
            ...
            setGlobalError('')
            try {
              await loginWithGoogle({ access_token: response.access_token })
              onSuccess?.()
            } catch (err) {
              setGlobalError(err.response?.data?.error || 'Đăng nhập Google thất bại, vui lòng thử lại.')
            } finally {
      ...
      setGlobalError('Dịch vụ Google đang tải, vui lòng thử lại sau vài giây.')
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5" noValidate>
      ...
      <button
        ...
        disabled={submitting}
        className="w-full h-12 rounded-xl border border-slate-200 bg-white text-slate-700 font-semibold text-sm hover:bg-slate-50 hover:border-slate-300 active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
      >
        <GoogleIcon />
